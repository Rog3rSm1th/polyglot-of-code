...
# Execute an instruction and return the grid
function execute(ins::Instruction, grid)

    # increase brightness by 2
    if ins.instruction == "toggle"
        grid[ins.start_x:ins.start_y,ins.end_x:ins.end_y] = map(x -> x + 2, grid[ins.start_x:ins.start_y,ins.end_x:ins.end_y])
    # decrease brightness by 1
    elseif ins.instruction == "turn off"
        grid[ins.start_x:ins.start_y,ins.end_x:ins.end_y] = map(x -> max(0, x - 1), grid[ins.start_x:ins.start_y,ins.end_x:ins.end_y])
    # increase brightness by 1
    else
        grid[ins.start_x:ins.start_y,ins.end_x:ins.end_y] = map(x -> x + 1, grid[ins.start_x:ins.start_y,ins.end_x:ins.end_y])
    end
